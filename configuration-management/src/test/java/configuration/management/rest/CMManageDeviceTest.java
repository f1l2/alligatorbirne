...
public class CMManageDeviceTest extends AbstractTestRestCM {

    @Test
    public void getAll1() {

        when().get(RESOURCE_NAMING.CM_GET_ALL_DEVICES.getPath())
    ...
    public void getAll2() {
        ...
        Response response = get(RESOURCE_NAMING.CM_GET_ALL_DEVICES.getPath());
    ...
    public void registerDataSources() {
        ...
        Response response = given().body(dataSources).contentType("application/json").post(ResourceUtils.getPath(RESOURCE_NAMING.CM_REGISTER_DEVICE_SOURCES, connection.getId()));
    ...
    public void heartbeat() {
        ...
        Response response = given()
                //
                .when().put(ResourceUtils.getPath(RESOURCE_NAMING.CM_HEART_BEAT_DEVICE, connection.getId()));
    ...
    public void heartbeatFailMissingRegistration() {
        ...
        Response response = given()
                //
                .when().put(ResourceUtils.getPath(RESOURCE_NAMING.CM_HEART_BEAT_DEVICE, "1234"));
    ...
    public void getDataSourcesByDevice() {
        ...
        Response response = given().body(dataSources).contentType("application/json").post(ResourceUtils.getPath(RESOURCE_NAMING.CM_REGISTER_DEVICE_SOURCES, register.getId()));

        assertEquals(HttpStatus.OK.value(), response.getStatusCode());

        // ask for data sources

        response = given().get(ResourceUtils.getPath(RESOURCE_NAMING.CM_GET_DEVICE_DATA_SOURCES, register.getId()));
    ...
    public void getDevicesByDataSource() {
        ...
        String path = ResourceUtils.getPath(RESOURCE_NAMING.CM_GET_DEVICE_BY_DATA_SOURCES, "device" + count, "domain" + count);
    ...
    private Connection register() {
        ...
        Response response = given().body(connection).contentType("application/json")
                //
                .when().post(RESOURCE_NAMING.CM_REGISTER_DEVICE.getPath());
